...
const GererPrelev = ({ user }) => {
  ...
  const [detailsModeA, setdetailsModeA] = useState({active: false,rowId: null,});
  ...
  const seeDetailsA = (id) => {
    setdetailsModeA({ active: true, rowId: id });
  };
...
if (detailsModeA.active) {
    return (
      <PrelevDetails
        user="agent"
        id={detailsModeA.rowId}
        exitdetailsMode={() => setdetailsMode({ active: false, rowId: null })}
      />
    );
  }
