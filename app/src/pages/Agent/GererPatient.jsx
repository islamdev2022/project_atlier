...
import AjouterPrelev from "./AjouterPrelev";
const GererPatient = () => {
  const rowData = [
  ...
  return (
    <div className="flex flex-col items-center">
      <p className="text-2xl xl:text-3xl text-green font-bold relative top-3 ">
        List des Patients
      </p>
          ...
          <table className=" text-xl bg-gradient-to-t text-slate-600 font-semibold rounded-tl-lg w-full ">
            ...
            <tbody>
              {rowData.map((row, index) => (
                <tr
                  key={index}
                  className="h-16 cursor-pointer hover:bg-slate-200 border-y text-center"
                >
                  <td>{row.N}</td>
                  <td>{row.nom}</td>
                  <td>{row.prenom}</td>
                  ...
                  <td>
                    ...
                    >
                      ...
                      <div>
                        <BiCommentAdd
                          className=" cursor-pointer hover:bg-slate-300 size-8 p-1 rounded-xl hover:text-emerald-500"
                          onClick={() => AddDetails(row.N)}
                          onMouseEnter={() => {
                            setIsHoveredAP(index);
                          }}
                        ...
                        {isHoveredAP === index && (
                          <p
                            className="text-sm p-1 rounded bg-emerald-200 text-emerald-500 absolute mt-5 "
                            style={{
                              left: `${mousePosition.x}px`,
                              top: `${mousePosition.y}px`,
                            }}
              ...
              {deleteMode.active && (
                <AlertModal
                  option={"Voulez-vous Suprimer cette patient ?"}
